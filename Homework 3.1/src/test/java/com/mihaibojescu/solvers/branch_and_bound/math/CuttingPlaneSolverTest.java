...
public class CuttingPlaneSolverTest {
    @Test
    public void toyExample() throws Exception {
        Problem problem = new Problem(
                new Matrix(new double[] { 5, 4 }), new Matrix(new double[][] { { 1, 1 }, { 2, 1 }, }),
                new Matrix(new double[] { 5, 8 }));
        Solver cuttingPlaneSolver = new CuttingPlaneSolverAdapter();
        Result result = cuttingPlaneSolver.run(problem);

        assertArrayEquals(result.getSolution(), new double[] { 3.0, 2.0 });
        assertEquals(result.getObjectiveValue(), 23.0);
    }
